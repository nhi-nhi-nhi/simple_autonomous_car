...
class CarLogic:
    def __init__(self):
        ...
        self.max_see_sign_time = 5
        self.stop = False
        self.buffer_time = 2

        self.pid_controller = PIDController(kp=0.025, ki=0.007, kd=0.005, dt=1/8)
        # Example of a more gradual PID control setup
        # self.pid_controller = PIDController(kp=0.015, ki=0.007, kd=0.01, dt=1/8)
    ...
    def execute_turning(self):
        ...
        if self.turning_time and (0 <= (time.time() - self.last_sign_time) <= self.turning_time):
            self.perform_turn()
            if self.len_line == 2 and (time.time() - self.last_sign_time) >= self.min_turn_time + self.buffer_time:
