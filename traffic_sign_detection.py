...
def calculate_sign_distance(sign, image_width, image_height):
    """
    ...
    """
    _, x_min, y_min, x_max, y_max = sign

    # Calculate the center point of the detected sign
    sign_center_x = (x_min + x_max) // 2
    sign_center_y = (y_min + y_max) // 2

    # Use the y-coordinate of the sign to determine how close it is
    # The closer the sign is to the bottom of the image, the closer it is to the car
    distance = image_height - sign_center_y

    # Optionally, we could also factor in the size of the sign (area of the bounding box)
    sign_width = x_max - x_min
    sign_height = y_max - y_min
    sign_area = sign_width * sign_height

    # Normalize distance by image height
    normalized_distance = distance / image_height
